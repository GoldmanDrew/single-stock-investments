# CLAUDE.md â€” single-stock-investments

## IB Gateway coexistence (three systems, one Gateway) â€” DO NOT VIOLATE
...
| System | IDs |
|---|---|
| spx-0dte | **17** (live executor), **18** (ibc_guard handshake probe â€” read-only connect/disconnect, never subscribes or transmits) |
| ls-algo | 0 (cancel coordinator, orderRef-filtered), 41 (rebalancer base), 77 (flow program), 90 (daily screener), 197/207 (bucket5 probes), workers 241â€“273, 341â€“373, 551, 1041+ |
| this repo | 71/72/73 (sleeves drew/michael/sync), 81 (collector, read-only), 82 (master observer, reserved â€” **not 90**, which is ls-algo's screener), **91** (order bridge, sole hub transmitter) |
| Operator TWS | manual only |

Rules that keep SPX safe (test-enforced where noted):

1. **Never `reqGlobalCancel`** â€” cancels every working order account-wide,
...
2. **Never take another system's client ID** â€” the hazard is grabbing an ID
...
4. **Market-data lines are one account-wide pool** shared with SPX's option
   ...
   and is leak-tested (50 quotes â†’ 0 open lines, â‰¤1 concurrent). Keep it
   that way; never add a streaming subscription to the hub.
5. **`reqAutoOpenOrders` must never appear in this repo** (test-enforced) â€”
...
6. **Never stop/restart the Gateway process** or flip its global Read-Only
   ...
   **Sole carve-out (2026-08-20):** spx-0dte's `ibc_guard` may issue ONE
   ...
   accepts TCP, client-18 probe handshake fails â‰¥2 consecutive 5-min checks
   during session hours). In that state every system on this Gateway is
   equally disconnected, so the restart harms no one â€” it pages Slack when it
...
7. **Background jobs deployed to NY4** (Whisper backfill, collectors) run
   `Niceâ‰¥15` + `CPUQuota` so the SPX executor never waits on CPU.
