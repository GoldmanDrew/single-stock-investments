...
def connect_ib(owner: str, cfg: Mapping[str, Any] | None = None, *, paper: bool = False):
    cfg = cfg or load_config()
    IB, _, _ = _import_ib()
    ibkr = cfg["ibkr"]
    port = int(ibkr["paper_port"] if paper else ibkr["live_port"])
    client_id = int((ibkr.get("client_ids") or {})[owner])
    ib = IB()
    ib.connect(str(ibkr.get("host") or "127.0.0.1"), port, clientId=client_id)
...
def fetch_positions(ib: Any, account_id: str) -> list[dict[str, Any]]:
    rows = []
    for pos in ib.positions(account_id) if account_id else ib.positions():
        if account_id and str(pos.account) != account_id:
            continue
        c = pos.contract
        rows.append({
            "account": pos.account,
            ...
            "avgCost": float(getattr(pos, "avgCost", 0) or 0),
            "orderRef": "",
        })
    return rows
