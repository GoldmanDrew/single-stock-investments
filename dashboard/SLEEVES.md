...
1. **Residual long-term names.** Michael's tab is Magis `U805366` after dropping the ls-algo universe (unless the name is a blacklist family Michael trades by hand) and SPX / XSP. Drew starts empty and only receives fills tagged `DREW_SLEEVE`.
...
**IB sync** (`python -m _system.trading.sleeves.sync_ib`) reads account `U805366` the same way ls-algo and SPX 0DTE do (TWS `127.0.0.1:7496`, or `--flex` OpenPositions XML). HMAC `POST /api/v1/sleeves/ingest` updates D1 when `SLEEVE_INGEST_TOKEN` is set.
